...
2. **会话配置开录瞬间冻结**（分段模式 / AI 整理 / 逐字注入 / 剪贴板），中途改设置不影响本轮。
   唯一例外：听写录音落盘后的有效时长不足 5 秒（`SKIP_REFINE_BELOW_MS`，按音频算、不按按键时长）时，Rust 关掉本轮 AI 整理，直接输出转写原文——短句整理前后几乎一样，不值得多等一次 LLM。翻译不受影响。改后的配置随 `process` 事件下发给前端。
...
### 注入
1. **注入落点 = 松手瞬间的前台焦点**（不再是按下快捷键时的快照），支持"在 A 起录、切到 B 的输入框落字"的常见用法。两层策略：
   - 流式逐字注入被 gate 在**录音起点**那个 app（中途切走的 app 不会飞进字，典型：录 A 中途切 B 看消息）；
   - 松手后整段 paste 兜底打到**当下**前台焦点，落点能否写入由注入前 AX 现场判定兜底（不可写则降级剪贴板 + Toast）；
   - **历史 `target_app` / `focus_title` 与 AI refine 排版按真实落点（松手瞬间前台 app）记录**，不是录音起点——用户在 A 起录、切到 B 落字时，历史与 AI 排版都反映 B。注入机器内部（流式 gate / axUnreliable / tail 路由）仍用起点快照，二者职责不同。
