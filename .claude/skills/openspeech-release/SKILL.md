...
> **本文件是导航 + 标准 stable 流程的可执行指引。** 特殊场景（beta、跳 beta、COS、撤回等）
...
本地 commit → 写 docs/changelogs/{ver}/zh.md → pnpm version <seg>
   ...
   → CI（6 平台 build → release job 拼 latest.json → 镜像 COS）
   → 人工 publish draft → updater 拉 manifest（指 COS）→ 终端用户收到更新
```

完整流程图、SSoT 表、文件索引、CI 各 step 细节看 `references/architecture.md`。
...
- Updater 运行时 endpoint SSoT = `src-tauri/src/update_channel.rs`（`tauri.conf.json` 里的 `endpoints` 只是占位）
...
| 国内下载慢 / COS / manifest 指哪儿 / updater 日志 | COS 镜像链路 | `references/cos-mirror.md` |
...
返回 `Not Found` → publish 没成功，重做。

COS 双 manifest 验证、updater 日志路径、Tauri 2 产物格式注意点见
`references/cos-mirror.md` 和 `references/troubleshooting.md`。
...
**Beta 专属、跳 beta 专属、COS 专属的 Common Mistakes** 分别在
`references/beta.md` / `references/skip-beta.md` / `references/cos-mirror.md` 末尾。
...
| 国内 COS 镜像 / updater 日志 / 应急 | `references/cos-mirror.md` |
...
> `sync-version.mjs` / `docs/changelogs/` 路径约定 / 腾讯云 COS 链路有变更时，
