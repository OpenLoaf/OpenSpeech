...
| 「重发一下」「补一个 build」「重跑 CI」 | tag 已存在，重新触发 | 见 §六「重跑 CI」 |
| 「撤回 / 下架 / 把那个版本删了」 | unpublish 已发布 Release | 见 §六「撤回」 |
...
| `src-tauri/tauri.conf.json` | `plugins.updater.{pubkey,endpoints}` + `bundle.createUpdaterArtifacts: false`（CI override true） |
| `src-tauri/src/lib.rs` | `tauri_plugin_updater::Builder::new().build()` 注册 + 托盘"检查更新"菜单项 |
| `src/main.tsx` | bootPromise 启动检查（5s 超时） |
