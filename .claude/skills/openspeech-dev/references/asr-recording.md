...
> 何时读：改录音、改 STT、调 SaaS realtime ASR 集成、改触发录音的 gate 逻辑、加新 provider。
> 真相来源：`src-tauri/src/stt/mod.rs` + `src-tauri/src/audio/` + `src/lib/stt.ts` + `src/stores/recording.ts`。事件名 / 命令名 / payload 直接读源码。
