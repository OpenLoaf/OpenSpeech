...
| 模型 | **BYO-Model**：OpenSpeech 不内置模型，用户在设置里配置 REST STT 端点（API URL + Key） |
| 隐私立场 | 录音不落盘；历史与词典仅本地；除用户主动配置的 REST 端点外不向任何服务器发送数据 |
...
| `rdev` | 0.5.3（desktop-only target，crates.io 上游 Narsil/rdev） | 全局键盘监听：modifier-only 绑定（Fn / Ctrl+Win / Right Alt 按住触发，见 `src-tauri/src/hotkey/modifier_only.rs`），以及未来的 Esc 按状态订阅、PTT 松开 keystate 轮询兜底。**`rdev::listen` 只能调一次**，所以新的订阅需求必须扩展 modifier_only 模块而不是另起 listen。0.5.3 在 macOS 原生支持 `Key::Function`（内部走 CGEventTap）；Windows / Linux 的 Fn 无硬件支持，切 fork 也无解。 |
...
**非 Tauri 插件的 Rust 依赖已装**（见上"Rust" 表）：`rdev / cpal / hound / tokio / reqwest / enigo / keyring / zeroize`。用 `cargo add --manifest-path src-tauri/Cargo.toml <crate>` 继续添加新 crate；**不要**用 `pnpm tauri add`（那是 Tauri 插件专用）。
...
已建立的关键子目录（按本表展开）：
- `src/stores/` —— Zustand stores：`hotkeys.ts`（快捷键） / `recording.ts`（录音状态机） / `settings.ts`（通用 + 个性化设置，tauri-plugin-store 持久化，schemaVersion 2）
- `src/lib/secrets.ts` —— 机密 invoke 封装（`setSecret` / `getSecret` / `deleteSecret`），对应 `src-tauri/src/secrets/`
- `src/lib/audio.ts` —— 麦克风 invoke 封装（`listInputDevices` / `startAudioLevel` / `stopAudioLevel`），对应 `src-tauri/src/audio/`；start/stop 必须成对调用，Rust 侧靠 ref_count 复用 stream
- `src-tauri/src/hotkey/` —— 全局快捷键编排。`mod.rs` 处理 `combo` 绑定走 `tauri-plugin-global-shortcut`，`modifier_only.rs` 处理 `modifierOnly` 绑定走 `rdev::listen`（Fn / Ctrl+Win / Right Alt 等按住触发）。`apply_bindings` 按 `binding.kind` 分流到两条路径，emit 统一的 `HOTKEY_EVENT` payload。**`rdev::listen` 进程生命周期内只能调一次**——Esc 订阅 / PTT keystate 轮询等未来扩展必须复用 `modifier_only.rs` 的 listen 线程，不能另起一个。
- `src-tauri/src/secrets/` —— 系统密钥链 (keyring) 命令 `secret_set` / `secret_get` / `secret_delete`
- `src-tauri/src/audio/` —— cpal 麦克风电平采集（仅电平监控，不负责录音→WAV）；命令 `audio_level_start(device_name?)` / `audio_level_stop` / `audio_list_input_devices`；事件 `openspeech://audio-level`（20Hz, 0..1 peak）；引用计数支持多处并发订阅（录音流程 + 设置页实时电平），切设备时保持 ref_count 强制 restart stream

规划中但尚未建立（在开发对应功能时再建，并同步本表）：
- `src/lib/tauri.ts` —— invoke 的前端封装
- `src/locales/` —— i18n 文案
- `src-tauri/src/audio/` 的 **录音→WAV** 路径（当前只做 peak 监控）
- `src-tauri/src/inject/` —— 文本注入（剪贴板 + enigo 两路）
- `src-tauri/src/stt/` —— REST 调用与 Provider trait
- `src-tauri/src/db/` —— SQLite schema 与迁移
